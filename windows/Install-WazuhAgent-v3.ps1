...
[CmdletBinding()]
param(
    # Mandatory parameter forces the user to choose an Agent Name upon execution
    [Parameter(Mandatory = $true, HelpMessage = "You must specify a unique name for this Wazuh Agent.")]
    [string]$AgentName,
...
$STUNNEL_SVC   = "stunnel" # Verified internal service name for stunnel
...
# CRITICAL FIX: Create temp directory BEFORE trying to initialize the log file
...
$stunnelCleanPath = $STUNNEL_CA -replace '\\','/'
$stunnelConf = @"
...
if (-not $svc) {
    ...
    Start-Process -FilePath "$STUNNEL_DIR\bin\stunnel.exe" -ArgumentList "-install" -NoNewWindow -Wait
    Start-Sleep -Seconds 3 # Added slightly longer delay for registration hook
    $svc = Get-Service -Name $STUNNEL_SVC -ErrorAction SilentlyContinue
    if (-not $svc) { Write-Fail "Failed to register stunnel service" }
}

# Safeguard configuration settings
